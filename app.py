...
# Define local path for NLTK data to avoid Cloud permission errors
...
def create_pdf_bytes(ticker, info, rating, thesis, peers_df, intrinsic_val):
    ...
    if peers_df is not None and not peers_df.empty:
        ...
        try:
            ...
            pdf.chapter_body("Detailed peer data available in dashboard.")
    ...
    if df is not None and not df.empty:
        ...
        with active_tabs[news_idx]:
            if news:
                valid_articles = 0
                for article in news:
                    # Filter: Ensure Title and Link exist
                    title = article.get('title')
                    ...
                    st.caption(f"Source: {publisher} | Score: {score:.2f}")
                    st.divider()
                    valid_articles += 1
                
                if valid_articles == 0:
                    st.info("No text-based news articles found (only videos/ads detected).")
            else: 
                st.warning("No news feed available.")
    else:
